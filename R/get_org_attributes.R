...
get_closed_non_empty_milestone_objects <- function() {
  debug(
    .le$logger,
    glue::glue(
      "Retrieving Milestone(s) in organization {.le$org}, repo {.le$repo}..."
    )
  )
  ...
  info(
    .le$logger,
    glue::glue(
      "Retrieved {length(closed_milestone_objects)} closed Milestone(s) in repo {.le$repo}"
    )
  )
  closed_non_empty_milestone_objects <- get_non_empty_milestone_objects_from_milestone_objects(
    closed_milestone_objects
  )

  info(
    .le$logger,
    glue::glue(
      "Retrieved {length(closed_non_empty_milestone_objects)} closed non-empty Milestone(s) in repo {.le$repo}"
    )
  )
...
get_open_non_empty_milestone_objects <- function() {
  ...
  info(
    .le$logger,
    glue::glue(
      "Retrieved {length(open_milestone_objects)} open Milestone(s) in repo {.le$repo}"
    )
  )
  open_non_empty_milestone_objects <- get_non_empty_milestone_objects_from_milestone_objects(
  ...
  info(
    .le$logger,
    glue::glue(
      "Retrieved {length(open_non_empty_milestone_objects)} open non-empty Milestone(s) in repo {.le$repo}"
    )
  )
...
get_all_non_empty_milestone_objects <- function() {
  ...
  info(
    .le$logger,
    glue::glue(
      "Retrieved {length(all_milestone_objects)} Milestone(s) in repo {.le$repo}"
    )
  )
  all_non_empty_milestone_objects <- get_non_empty_milestone_objects_from_milestone_objects(
  ...
  info(
    .le$logger,
    glue::glue(
      "Retrieved {length(all_non_empty_milestone_objects)} non-empty Milestone(s) in repo {.le$repo}"
    )
  )
  return(rev(all_non_empty_milestone_objects))
}

get_closed_milestone_objects_from_all_milestone_objects <- function(
  milestone_objects
) {
  purrr::keep(milestone_objects, ~ .x$state == "closed")
}

get_open_milestone_objects_from_all_milestone_objects <- function(
  milestone_objects
) {
  purrr::keep(milestone_objects, ~ .x$state == "open")
}

get_milestone_object_from_milestone_name <- function(
  milestone_name,
  milestone_objects
) {
...
get_grouped_milestone_names <- function(
  grouped_milestones,
  closed_titles = character()
) {
...
parse_remote_url <- function(remote_url) {
  ...
  if (stringr::str_detect(remote_url, "@")) {
    ...
    remote_url <- stringr::str_replace(
      remote_url,
      "^git@([^:]+):",
      "https://\\1/"
    )
    ...
    error(
      .le$logger,
      glue::glue("Remote url: {remote_url} doesn't match ssh or https formats")
    )
    rlang::abort(glue::glue(
      "Remote url: {remote_url} doesn't match ssh or https formats"
    ))
  }
...
get_remote <- function() {
  ...
  debug(
    .le$logger,
    glue::glue("Retrieved list of remotes: \n{debug_remote_list_str}")
  )
  ...
  remote <- {
    ...
    if (num_remotes == 1) {
      ...
      debug(
        .le$logger,
        glue::glue("Single remote: Selected only remote in list")
      )
      remote_list[1, ]
      ...
      info(
        .le$logger,
        glue::glue(
          "Retrieving remote name from GHQC_REMOTE_NAME environment variable: {remote_env_var}"
        )
      )
      ...
      info(
        .le$logger,
        "No GHQC_REMOTE_NAME environment variable found. Using \"origin\" from list of remotes."
      )
      ...
      tryCatch(
        {
          ...
          info(
            .le$logger,
            glue::glue(
              "No GHQC_REMOTE_NAME environment variable found. Using first remote from list of remotes: {remote_list$name[1]}"
            )
          )
          remote_list[1, ]
...
get_all_issues_in_repo <- function() {
  debug(
    .le$logger,
    glue::glue("Retrieving all Issue(s) from repo: {.le$repo}...")
  )
  open_issues <- list()
  page <- 1
  ...
  info(
    .le$logger,
    glue::glue("Retrieved {num_issues} Issue(s) from repo: {.le$repo}")
  )
  return(issues)
}
...
get_all_issues_in_milestone_from_milestone_number <- function(
  org = .le$org,
  repo = .le$repo,
  api_url = .le$github_api_url,
  milestone_number,
  ...
  info(
    .le$logger,
    glue::glue(
      "Retrieved {length(issues)} ghqc Issue(s) from Milestone: {milestone_name}"
    )
  )
  return(issues)
}
...
get_milestone_url <- function(milestone_name) {
  milestone_number <- get_milestone_number(list(
    org = .le$org,
    repo = .le$repo,
...
get_collaborators <- function() {
  tryCatch(
    ...
    error = function(e) {
      error(
        .le$logger,
        glue::glue("No collaborators found in {.le$org}/{.le$repo}")
      )
      rlang::abort(conditionMessage(e))
    }
  )
